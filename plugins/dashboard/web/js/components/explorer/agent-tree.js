/**
 * Agent Tree Component - Store-connected tree view of agents grouped by domain
 * @module components/explorer/agent-tree
 */

import { LitElement, html, css } from 'lit';
import { repeat } from 'lit/directives/repeat.js';
import { SignalWatcher } from '../core/signal-watcher.js';
import { AppStore, Actions, filteredAgents, agentsByDomain } from '../../store/app-state.js';
import { AgentService } from '../../services/agent-service.js';
import './agent-item.js';

class AgentTree extends SignalWatcher(LitElement) {
    static properties = {
    ...
    static styles = css`
        ...
        .group-icon { width: 16px; height: 16px; color: var(--accent-color, #4a90d9); }
    ...
    connectedCallback() {
        ...
        if (this.useStore) {
            ...
            this.watchSignals([
                AppStore.agents,
                AppStore.loadingAgents,
                AppStore.agentFilter,
                AppStore.selectedAgent,
                AppStore.agentExpandedGroups
            ]);

            // Load agents if store is empty
            if (AppStore.agents.value.length === 0 && !AppStore.loadingAgents.value) {
                AgentService.fetchAgents();
            }
        }
    }

    // Get agents from store or props
    _getAgents() {
        if (!this.useStore) {
            return this.agents;
        }
        // Use store's filtered agents if filter is active
        return AppStore.agentFilter.value
            ? filteredAgents.value
            : AppStore.agents.value;
    }
    ...
    _getSelectedId() {
        ...
        const selected = AppStore.selectedAgent.value;
    ...
    _isLoading() {
        ...
        return AppStore.loadingAgents.value;
    }
    ...
    _getFilteredAgents() {
        ...
        return agents.filter(agent => {
            ...
            const domain = (agent.domain || agent.plugin || '').toLowerCase();
            return name.includes(lowerFilter) || role.includes(lowerFilter) || domain.includes(lowerFilter);
        });
    }
    ...
    _groupAgents(agents) {
        ...
        if (this.useStore && !this._getFilter()) {
            return agentsByDomain.value;
        }

        // Manual grouping for filtered or prop-based data
        const groups = {};
        agents.forEach(agent => {
            const group = agent.domain || agent.plugin || 'Other';
    ...
    _toggleGroup(groupName) {
        if (this.useStore) {
            Actions.toggleAgentGroup(groupName);
        } else {
            this._localExpandedGroups = {
                ...this._localExpandedGroups,
    ...
    _isGroupExpanded(groupName) {
        if (this.useStore) {
            return AppStore.agentExpandedGroups.value[groupName] !== false;
        }
        return this._localExpandedGroups[groupName] !== false;
    }

    // Handle agent selection
    _handleAgentSelect(e) {
        const agent = e.detail.agent;

        if (this.useStore) {
            Actions.setSelectedAgent(agent);
        } else {
            this.selectedId = agent?.id || agent?.name;
        }
    ...
    _handleAgentOpen(e) {
        this.dispatchEvent(new CustomEvent('agent-open', {
            detail: e.detail,
            bubbles: true,
            composed: true
        }));
    ...
    _renderEmpty() {
        ...
        return html`
            <div class="empty-state">
                ...
                <div class="empty-text">${filter ? 'Try a different search term' : 'Agents will appear here'}</div>
            </div>
        `;
    }

    _renderGroup(groupName, agents) {
            ...
            <div class="group">
                ...
                ${isExpanded ? html`
                    <div class="group-items">
                        ${repeat(agents, a => a.id || a.name, a => html`
                            <agent-item
                                .agent=${a}
                                ?selected=${(a.id || a.name) === selectedId}
                                @agent-select=${this._handleAgentSelect}
                                @agent-open=${this._handleAgentOpen}
                            ></agent-item>
                        `)}
                    </div>
                ` : ''}
    ...
    render() {
        ...
        if (this.groupByDomain) {
            const groups = this._groupAgents(filtered);
            return html`<div class="tree-container">${groups.map(([name, agents]) => this._renderGroup(name, agents))}</div>`;
        }

        return html`
            <div class="tree-container">
                ${repeat(filtered, a => a.id || a.name, a => html`
                    <agent-item
                        .agent=${a}
                        ?selected=${(a.id || a.name) === selectedId}
                        @agent-select=${this._handleAgentSelect}
...
customElements.define('agent-tree', AgentTree);
export { AgentTree };
