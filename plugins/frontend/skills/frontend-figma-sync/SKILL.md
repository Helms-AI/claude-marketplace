...
When invoked with `/frontend-figma-sync`, synchronize design decisions between Figma and code through variables, tokens, and design specs.
...
|------------|------------------|
| `/frontend-design-system` | Token definitions, naming conventions |
| `/frontend-component-architect` | Component structure, prop mappings |

### Context This Skill Provides

| To Skill | Context Provided |
|----------|------------------|
| `/frontend-orchestrator` | Sync status, token changes |
| `/frontend-design-system` | Figma updates, new tokens |

### Announcing Context Transfer

When passing context to another skill, announce:
```
"**Cameron Reyes → Sam Rivera:** Figma sync complete. [X] tokens exported, [Y] components mapped. [Status/next steps]."
```
...
- **Sam Rivera (Systems):** Token naming conventions and structure
- **Alex Kim (Architecture):** Component generation patterns
- **Taylor Brooks (Performance):** Token file size optimization
