...
When invoked with `/frontend-data-grid`, design data-dense interfaces that are scannable, sortable, and accessible across all device sizes.
...
|------------|------------------|
| `/user-experience-aesthetic-director` | Visual tone, density preference |
| `/frontend-orchestrator` | User's original request, data structure |
| `/user-experience-layout-composer` | Available space, responsive requirements |

### Context This Skill Provides

| To Skill | Context Provided |
|----------|------------------|
| `/frontend-component-architect` | Table/grid components, cell types |
| `/frontend-accessibility-auditor` | Table semantics, sortable headers |
| `/frontend-performance-engineer` | Virtualization needs, row count |
| `/frontend-responsive-engineer` | Mobile table strategy |

### Announcing Context Transfer

When passing context to another skill, announce:
```
"**Reese Kim → Taylor Brooks:** Here's the data grid specification:
