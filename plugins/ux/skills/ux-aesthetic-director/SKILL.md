...
## Next Steps
1. → `/ux-typography-curator` for font selection
2. → `/ux-color-alchemist` for palette creation
3. → `/ux-design-system` for token foundation
```
...
- Before `/ux-design-system` to inform token personality
- Before `/ux-component-architect` to guide component styling
- When designs feel "generic" and need direction

### Skills That Consume Aesthetic Brief
- `/ux-typography-curator` → Uses typography direction
- `/ux-color-alchemist` → Uses color direction and tone
- `/ux-design-system` → Implements token personality
- `/ux-layout-composer` → Uses spatial direction
- `/ux-texture-atmosphere` → Uses aesthetic archetype
- `/ux-micro-delight` → Uses motion personality
- `/ux-motion-designer` → Uses motion direction
